...
if (config.doBackgroundWork) {
  const worker = new Worker(
    ...
    async (job: Job) => {
      ...
      if (![1, 4, 5, 10, 324, 42161].includes(config.chainId)) {
        throw new Error("Unsupported network");
      }

      if (
        !["nftearth", "opensea", "looks-rare", "x2y2", "universe", "infinity", "flow"].includes(
          orderbook
        )
      ) {
  ...
  switch (orderbook) {
    case "nftearth":
      return new RateLimiterRedis({
        ...
        points: NFTEarthApi.RATE_LIMIT_REQUEST_COUNT,
        duration: NFTEarthApi.RATE_LIMIT_INTERVAL,
      });
    case "looks-rare":
      return new RateLimiterRedis({
        storeClient: rateLimitRedis,
