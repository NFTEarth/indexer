...
import { offChainCheck } from "@/orderbook/orders/flow/check";
...
export const save = async (orderInfos: OrderInfo[], relayToArweave?: boolean) => {
  ...
  const handleOrder = async ({ orderParams, metadata }: OrderInfo) => {
    try {
      const order = new Sdk.Flow.Order(config.chainId, orderParams);
      const id = order.hash();
      ...
      const feeBreakdown = [
        {
          kind: "marketplace",
          recipient: Sdk.Flow.Addresses.Exchange[config.chainId],
          bps: FEE_BPS,
        },
      ];
      ...
      let source = await sources.getOrInsert("flow.so");
      if (metadata.source) {
      ...
      orderValues.push({
        ...
        conduit: toBuffer(Sdk.Flow.Addresses.Exchange[config.chainId]),
        fee_bps: FEE_BPS,
  ...
  if (orderValues.length) {
    ...
    if (relayToArweave) {
      await arweaveRelay.addPendingOrdersFlow(arweaveData);
    }
  }
