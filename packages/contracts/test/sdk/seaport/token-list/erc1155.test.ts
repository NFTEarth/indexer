...
describe("Seaport - TokenList ERC1155", () => {
  ...
  it("Build and fill buy order with partial filling", async () => {
    ...
    await weth.approve(seller1, Seaport.Addresses.Exchange[chainId]);
    await weth.approve(seller2, Seaport.Addresses.Exchange[chainId]);
    ...
    await nft.approve(seller1, Seaport.Addresses.Exchange[chainId]);
    await nft.approve(seller2, Seaport.Addresses.Exchange[chainId]);

    const exchange = new Seaport.Exchange(chainId);
    const builder = new Seaport.Builders.TokenList(chainId);
