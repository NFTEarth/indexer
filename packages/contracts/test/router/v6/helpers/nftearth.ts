...
export const setupNFTEarthListings = async (listings: NFTEarthListing[]) => {
  ...
  for (const listing of listings) {
    ...
    if (nft.kind === "erc721") {
      ...
      await nft.contract
        .connect(seller)
        .setApprovalForAll(Sdk.NFTEarth.Addresses.Exchange[chainId], true);
    } else {
    ...
    const builder = new Sdk.NFTEarth.Builders.SingleToken(chainId);
    ...
    if (listing.isCancelled) {
      const exchange = new Sdk.NFTEarth.Exchange(chainId);
...
export const setupNFTEarthOffers = async (offers: NFTEarthOffer[]) => {
  const chainId = getChainId();
  ...
  for (const offer of offers) {
    ...
    await weth.approve(buyer, Sdk.NFTEarth.Addresses.Exchange[chainId]);
...
export type NFTEarthERC20Approval = {
  ...
  amount: BigNumberish;
  zone?: string;
  orders?: Sdk.NFTEarth.Order[];
};

export const setupNFTEarthERC20Approvals = async (
  approvals: NFTEarthERC20Approval[]
) => {
  const chainId = getChainId();
  ...
  for (const approval of approvals) {
    ...
    await erc20.approve(giver, Sdk.NFTEarth.Addresses.Exchange[chainId]);
...
export type NFTEarthERC721Approval = {
  giver: SignerWithAddress;
  filler: string;
  receiver?: string;
  nft: {
...
export const setupNFTEarthERC721Approvals = async (
  approvals: NFTEarthERC721Approval[],
  skipMint?: boolean
) => {
  const chainId = getChainId();
