...
import {
  NFTEarthERC20Approval,
  NFTEarthListing,
  setupNFTEarthERC20Approvals,
  setupNFTEarthListings,
...
describe("[ReservoirV6_0_0] NFTEarth listings", () => {
  ...
  const testAcceptListings = async (
    ...
    const listings: NFTEarthListing[] = [];
    ...
    await setupNFTEarthListings(listings);
    ...
    const executions: ExecutionInfo[] = [
                ...
                [
                  {
                    ...
                    fee: 500,
                    // Send USDC to the NFTEarth module
                    recipient: nftearthModule.address,
                    amountOut: listings
                      .map(({ price }, i) =>
      ...
      listingsCount > 1
        ? {
            ...
            data: nftearthModule.interface.encodeFunctionData(
  ...
  it("Permit2 - Fill ETH listing with USDC", async () => {
    ...
    const swapExecutions: ExecutionInfo[] = [
      // 1. Swap ETH for USDC on UniswapV3, sending the USDC to the NFTEarth module
      {
        module: uniswapV3Module.address,
    ...
    await setupNFTEarthListings([listing]);
    ...
    const executions: ExecutionInfo[] = [
      ...
      {
        module: nftearthModule.address,
        data: nftearthModule.interface.encodeFunctionData("acceptETHListing", [
          {
            parameters: {
              ...listing.order!.params,
              totalOriginalConsiderationItems:
    ...
    expect(ethBalancesAfter.nftearthModule).to.eq(0);
  ...
  it("Permit2 - Fill listing with USDC", async () => {
    ...
    const executions: ExecutionInfo[] = [
      ...
      {
        module: nftearthModule.address,
        data: nftearthModule.interface.encodeFunctionData("acceptERC20Listing", [
          {
            parameters: {
              ...listing.order!.params,
              totalOriginalConsiderationItems:
  ...
  it("Fill USDC listing approval-less", async () => {
    ...
    // the router is supposed to be stateless), we do create a NFTEarth
    // order which gives the funds to the router (eg. offer = USDC and
    // consideration = USDC - with the router as a private recipient).
    // This way, the USDC approval will be made on the NFTEarth conduit
    // and the router stays stateless.

    const approval: NFTEarthERC20Approval = {
      giver: bob,
    ...
    await setupNFTEarthERC20Approvals([approval]);
